...
- **Dedicated Tor UID Policy:** Bypasses Tor daemon traffic (`tor` / `debian-tor` UID) to prevent routing loops, paired with loopback leak suppression.
- **Strict IPv6 Blackout:** Unconditionally rejects host IPv6 traffic via `ip6tables -P DROP` and `net.ipv6.conf.all.disable_ipv6 = 1` to eliminate dual-stack clearnet exposure.
- **Optional Network Namespace (`veth` jail):** Isolates designated target processes into a clean kernel network namespace, initializing the virtual interface with a cryptographically randomized unicast MAC address (`(first_byte & 0x03) == 0x02`) via OS-CSPRNG before bringing links up.

### 2. Tor Access-Link L4 TCP Normalization (`wraith-net`)
- **Targeted NFQUEUE Interception:** Selectively captures outbound TCP SYN packets generated by the Tor daemon UID before transmission across the local access link to the first Tor Guard relay.
- **In-Flight SYN Option & Parameter Clamping:** Reorders TCP options to match standardized profiles, clamps MSS to 1460 bytes, enables SACK-permitted flags, and clears TCP Timestamps (`TSval/TSecr = 0`).
- **Standardized TTL Normalization:** Sets outbound IP TTL (e.g., 64 or 128) on Tor access packets, neutralizing passive OS fingerprinting (e.g., p0f) visible to local ISPs and network observers. *(Note: Normalizes local access link only; does not alter public Tor exit packets or replace the kernel TCP stack).*

### 3. Cryptographic DNS Engine & DNSSEC Validation (`wraith-guard`)
- **Port-53 Loopback Interception:** Binds a local resolver at `127.0.0.1:5354`, capturing both standard UDP and TCP port-53 queries.
- **Recursive DNSSEC Proof Verification:** Integrates the Hickory DNS engine to validate cryptographically signed DNSSEC resource records (RRSIG, DS, DNSKEY) down to root trust anchors, rejecting tampered or forged responses.
- **Tor DoH Encapsulation:** Routes upstream DNS-over-HTTPS queries strictly through the local Tor SOCKS5 proxy (`127.0.0.1:9050`).
- **FQDN Trailing-Dot Normalization:** Enforces RFC 1035 root dot normalization on all incoming host queries (`dns_engine.rs`), preventing search path leakage and NXDOMAIN hijacking.

### 4. Layer-7 HTTP Header Sanitization & TLS Impersonation (`wraith-tor`)
- **Local HTTP/CONNECT Proxy Relay:** Operates a local relay on `127.0.0.1:9055` providing sanitized forwarding into Tor SOCKS5.
- **1,338-Entry Signature Catalog:** Strips identifying client headers, non-browser metadata, and security tool fingerprints on initial cleartext requests.
- **BoringSSL Browser TLS Fingerprinting:** Features native BoringSSL integration to negotiate authentic Chrome, Firefox, and Safari ClientHello signatures, matching specific JA3/JA4 hashes and HTTP/2 parameter frames on Wraith-managed connections.
- **Cover Traffic Engine:** Injects authentic, periodic HTTPS cover requests over Tor at randomized 15–45 second intervals (`traffic_jitter.rs`) to obfuscate local traffic burst characteristics.

### 5. In-Memory Security, Sandboxing & Cryptographic Shredding (`wraith-core` / `wraith-forensic`)
- **Encrypted Ephemeral RAMFS Vault:** Implements an in-memory secret vault secured with RFC 8439 ChaCha20-Poly1305 AEAD, locked in physical RAM using `mlockall(MCL_CURRENT | MCL_FUTURE)` to prohibit swapping.
- **Process Memory Lockdown & Seccomp Jail:** Restricts process inspection via `PR_SET_DUMPABLE=0`, `PR_SET_NO_NEW_PRIVS=1`, and Seccomp BPF filters blocking unauthorized `ptrace` attachment and `process_vm_readv`/`process_vm_writev` cross-process memory extraction.
- **Zeroize-On-Drop Guarantees:** Ensures immediate memory wiping upon variable disposal using Rust's `Zeroize` and `ZeroizeOnDrop` traits for all cryptographic keys, ephemeral states, and sensitive buffers.
- **Multi-Pass Erasure:** Features a cryptographic file shredder (`shred.rs`), volatile cache flushing (`drop_caches`), verified swap partition clearing (`memory.rs`), and atomic shredding of ephemeral v3 Onion Service keys upon teardown (`onion_service.rs`).

### 6. Host Hardening, Panic Sentry & Durable Recovery (`wraith-host` / `wraith-cli`)
- **Reversible Kernel Sysctl Profiles:** Enforces host hardening (SYN cookies, reverse-path filtering, blocking ICMP broadcasts/timestamp responses) with atomic state snapshots for clean restoration.
- **L2 MAC Address & Hostname Cloaking:** Generates randomized CSPRNG MAC addresses (`mac.rs` using `OsRng`) and benign hostnames prior to DHCP renegotiation.
- **Fail-Closed Sentry & Watchdog:** Continuously monitors Tor daemon health; if Tor fails or an unhandled termination signal is caught, the Sentry immediately restores clean host network state, unlocks `/etc/resolv.conf`, and flushes session rules without leaving an unusable interface.
- **17-Language Interface & Diagnostics:** Fully localized interactive TUI and command suite with deep kernel/environment diagnostic auditing (`wraith doctor`).
