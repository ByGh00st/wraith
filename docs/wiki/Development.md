...
Follow the [release guide](https://github.com/ByGh00st/wraith/blob/main/docs/RELEASING.md) for version checks, native Debian packaging, musl builders and tag publication. The manual GitHub workflow builds artifacts without publishing a release. The [v1.4.0 release](https://github.com/ByGh00st/wraith/releases/tag/v1.4.0) uses the matching version tag and the same artifact validation steps.
