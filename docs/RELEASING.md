...
4. When publication is intended, create and push the matching tag from the reviewed commit:
   ...
   ```bash
   git tag -a v1.4.0 -m "Wraith v1.4.0"
...
The installer selects only stable releases with the exact expected asset names and official repository URLs. Checksums detect corruption/mismatches but share GitHub's release trust boundary. This workflow does not implement independent artifact signing or an APT repository. The `-u` CLI operation continues to update source checkouts only.
