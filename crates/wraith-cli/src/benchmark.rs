...
impl BenchmarkSuite {
    ...
    fn bench_chacha20_poly1305() -> BenchmarkResult {
        ...
        for _ in 0..iterations {
            if let Err(e) = chacha20_poly1305_encrypt(&key, &nonce, aad, &data) {
                eprintln!("Benchmark encryption error: {e}");
                break;
            }
        }
    ...
    pub fn print_report(results: &[BenchmarkResult]) {
        ...
        table.set_header(vec![
            ...
            Cell::new("DATA PROCESSED").add_attribute(Attribute::Bold).fg(Color::Cyan),
        ...
        for res in results {
            let data_str = if res.total_bytes >= 1024 * 1024 {
                format!("{:.2} MB", res.total_bytes as f64 / (1024.0 * 1024.0))
            } else if res.total_bytes >= 1024 {
                format!("{:.2} KB", res.total_bytes as f64 / 1024.0)
            } else {
                format!("{} B", res.total_bytes)
            };

            table.add_row(vec![
                Cell::new(res.engine).add_attribute(Attribute::Bold),
                Cell::new(format!("{}", res.iterations)),
                Cell::new(data_str).fg(Color::Cyan),
