...
impl NetlinkSocket {
    /// Returns the netlink socket process identifier
    pub fn pid(&self) -> u32 {
        self.pid
    }

    /// Returns the current netlink message sequence number
    pub fn seq(&self) -> u32 {
        self.seq
    }
    ...
    pub fn send_dump_request(&mut self, msg_type: u16, family: u8) -> Result<Vec<Vec<u8>>> {
        #[cfg(unix)]
        {
            ...
            // SAFETY: Casting valid stack structs hdr and ifinfo to byte slices of exact struct sizes.
            unsafe {
            ...
            // SAFETY: Sending valid buffer pointer over open netlink socket file descriptor.
            let send_res = unsafe {
    ...
    pub fn get_ifindex(ifname: &str) -> Result<i32> {
        #[cfg(unix)]
        {
            ...
            // SAFETY: Calling libc::if_nametoindex with null-terminated C string.
