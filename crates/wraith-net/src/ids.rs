...
impl PacketDissector {
    ...
    pub fn dissect(frame: &[u8]) -> Option<DissectedPacket> {
        ...
        // Leak Condition: Non-loopback packet escaping outside of Tor TransPort / Tor DNSPort
        let is_leak_suspect = !is_loopback && !is_tor_transport && !is_tor_dns && ethertype == ETHERTYPE_IPV4;
...
impl HttpToolSanitizer {
    ...
    pub fn sanitize_in_flight(payload: &mut [u8]) -> (usize, Option<String>) {
        let mut sanitized_count = 0;
        let mut unrecognized_alert = None;
            ...
            if matches_prefix {
                ...
                if !raw_ua_str.is_empty() {
                    ...
                    if is_known_offensive || !is_standard_browser {
                        ...
                        if !is_known_offensive && !is_standard_browser {
                            unrecognized_alert = Some(raw_ua_str.clone());
                        }
                    }
                }
        ...
        (sanitized_count, unrecognized_alert)
    }
}
...
impl EgressIntrusionDetector {
                    ...
                    loop {
                        ...
                        if res > 0 {
                            ...
                            if let Some(custom_ua) = unrecognized {
                                Self::print_custom_ua_alert(&custom_ua);
                            }
                            ...
                            if let Some(pkt) = PacketDissector::dissect(&buf[..n]) {
                                ...
                                if pkt.is_stun_leak {
                                    ...
                                    Self::print_leak_alert(
                                        "WebRTC STUN Discovery Probe",
                                        "Unauthorized STUN binding packet (attempts to resolve public IP)",
                                        "Trapped and destroyed at kernel boundary",
                                        "No action required - WebRTC leaks are actively neutralized by Wraith Shield."
                                    );
                                }
                                if pkt.is_leak_suspect {
                                    telemetry.clearnet_escapes_blocked.fetch_add(1, Ordering::SeqCst);
                                    let (src_str, dst_str) = if let Some(ref ip4) = pkt.ipv4 {
                                        (ip4.src_ip.to_string(), ip4.dst_ip.to_string())
                                    } else {
                                        ("Local".into(), "Clearnet".into())
                                    };

                                    Self::print_leak_alert(
                                        "Raw SYN / ICMP / Clearnet Egress Attempt",
                                        &format!("{src_str} ➔ {dst_str} (e.g. 'nmap -sS' or ping)"),
                                        "Destroyed at kernel fail-closed firewall (0 bytes leaked)",
                                        "Tor only tunnels Layer-4 TCP. For Nmap, switch to TCP Connect: 'nmap -sT -Pn -n -sV <target>'"
                                    );
                                }
                            }
                        } else {
    ...
    fn print_custom_ua_alert(raw_ua: &str) {
        let display_ua = if raw_ua.len() > 60 { &raw_ua[..60] } else { raw_ua };
        println!("\n  ╔════════════════════════════════════════════════════════════════════════════════════════════════╗");
        println!("  ║  ⚠️ WRAITH-DPI // UNRECOGNIZED CUSTOM USER-AGENT INTERCEPTED                                    ║");
        println!("  ╠════════════════════════════════════════════════════════════════════════════════════════════════╣");
        println!("  ║  • Detected Raw Header: {display_ua:<71} ║");
        println!("  ║  • Intercept Decision : Auto-sanitized to randomized authentic Browser User-Agent Pool         ║");
        println!("  ║  • Operational Rationale : Non-browser / bespoke User-Agents expose unique tracking markers!     ║");
        println!("  ║  • Operator Control   : To pass verbatim headers, configure dedicated transparent proxy rule.  ║");
        println!("  ╚════════════════════════════════════════════════════════════════════════════════════════════════╝\n");
    }

    fn print_leak_alert(vector: &str, target: &str, action: &str, remedy: &str) {
        println!("\n  ╔════════════════════════════════════════════════════════════════════════════════════════════════╗");
        println!("  ║  🚨 WRAITH-IDS // LEAK VECTOR INTERCEPTED & MITIGATED                                          ║");
        println!("  ╠════════════════════════════════════════════════════════════════════════════════════════════════╣");
        println!("  ║  • Intercepted Vector : {vector:<71} ║");
        println!("  ║  • Target / Traffic   : {target:<71} ║");
        println!("  ║  • Gateway Action     : {action:<71} ║");
        println!("  ║  • Recommended Fix    : {remedy:<71} ║");
        println!("  ╚════════════════════════════════════════════════════════════════════════════════════════════════╝\n");
    }
